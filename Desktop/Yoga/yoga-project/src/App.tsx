...
import withLiquidEtherBackground from "./components/withLiquidEtherBackground";
import Cards from "./components/Cards";
import IMG1 from './assets/dress1.jpeg';
...
import Description from './components/DressDescription';

// TODO: Lazy load, background is loaded lately, description UI, buy now, headers, infinite scroll, scrollsequence, host it
  ...
  <>
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around', width: '80%' }}>
        {dataDescription.map((dress) => {
          return (
            <Cards key={dress.dressCode} dressData={dress} />
          )
        })}
      </div>
    </div>
...
const MainContentWithBg = withLiquidEtherBackground(MainContent);

const AppRouter = () => (
  <Router>
    <Routes>
      <Route path="/" element={<MainContentWithBg />} />
      <Route path="/dress/:id" element={<Description />} />
    </Routes>
  </Router>
);
